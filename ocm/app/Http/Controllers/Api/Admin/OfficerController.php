...
class OfficerController extends Controller
{
    ...
    public function index(Request $request){
        ...
        $positions = isset( $request->positions ) && $request->positions != null && strlen( $request->positions ) > 0 ? explode(',',$request->positions) : false ;
        ...
        $organizations = isset( $request->organizations ) && $request->organizations != null && strlen( $request->organizations ) > 0 ? explode(',',$request->organizations) : false ;
        ...
        $officerIds = isset( $request->ids ) && $request->ids != null && strlen( $request->ids ) > 0 ? explode(',',$request->ids) : false ;
        ...
        $crud->setRelationshipFunctions([
            ...
            "people" => [
                'id','firstname' , 'lastname' , 'enfirstname' , 'enlastname' , 'gender' , 'dob' , 'pob' , 'image' , 'mobile_phone' , 'office_phone' , 'passport' , 'nid' , 'marry_status' , 'address' , 'email' , 'nationality' , 'national' , 'death' , 'body_condition' , 'body_condition_desp' ,
                // father
                'father_firstname' ,
                'father_lastname' ,
                'father_enfirstname' ,
    ...
    public function storeOfficer(Request $request){
        ...
        $officer = RecordModel::where([
            ...
            $query->where('id' , $request->organization_structure_position_id );
        })
        ->first();
        ...
        if( $people ){
            ...
            $officer = $people->officers()->create([
                'public_key' => md5( 
                    ...
                    $request->code  . 
                    $people->id .
                    $request->organization_structure_position_id .
                ...
                'countesy_id' => 0 , 
                'official_date' => strlen( $request->official_date ) > 7 ? \Carbon\Carbon::parse( $request->official_date )->format( 'Y-m-d' ) : '' ,
                'unofficial_date' => strlen( $request->unofficial_date ) > 7 ? \Carbon\Carbon::parse( $request->unofficial_date )->format( 'Y-m-d' ) : '' ,
                'leader' => 0 ,
            ...
            $account->update(['people_id' => $people->id]) ;
            $officer->update([
                'people_id' => $people->id ,
                'user_id' => $account->id ,
            ]);
    ...
    public function destroy(Request $request){
        ...
        if( $officer ){
            if( $officer->people != null ){
                $officer->people->delete();
            }
            if( $officer->user != null ){
                $officer->user->delete();
            }
            $officer->deleted_at = \Carbon\Carbon::now() ;
            $officer->save();
            // User does exists
            return response([
                'ok' => true ,
                'officer' => $officer ,
                'message' => 'គណនី '.$officer->people->lastname . ' ' . $officer->people->firstname .' បានលុបដោយជោគជ័យ !' ,
                'ok' => true 
                ],
                200
            );
