<template>
  <div
    class="ocm-float-menu-bottom fixed top-0 left-0 right-0 z-50 p-0 h-12 flex bg-blue-700"
  >
    <div
      class="absolute left-0 right-0 h-12 p-2 leading-8 text-white font-moul"
    >
      {{
        title != undefined && title != null && title.length > 0
          ? title
          : $store.state.activePage.title != ""
            ? $store.state.activePage.title
            : "មិនមានឈ្មោះ"
      }}
    </div>
    ...
    <div
      class="absolute right-0 h-12 p-2 w-12 sm:visible md:w-40 lg:w-40 xl:w-40 flex flex-row-reverse cursor-pointer"
      @click="$router.push('/profile')"
    >
      <div class="flex-none rounded-full overflow-hidden w-8 h-8">
        <div
          v-if="profilePicture != null"
          class="rounded-full border-gray-200 w-8 h-8 flex-none mx-auto overflow-hidden bg-center bg-no-repeat bg-cover bg-white"
          :style="
            'background-size: 80%; background-image: url(' +
            profilePicture +
            ');'
          "
        ></div>
    ...
    <div
      class="absolute left-0 h-12 p-2 w-14 sm:visible md:w-40 lg:w-40 xl:w-40 flex cursor-pointer"
    ...
    >
      ...
      <div
        class="flex-none h-8 font-moul text-right ml-1 text-gray-700 invisible sm:visible md:visible lg:visible xl:visible"
        style="overflow: hidden; white-space: nowrap; text-overflow: ellipsis"
      >
        <!-- <div class="h-3 text-left text-xxxxs font-moul  text-yellow-400 bg-white " >ទីស្ដីការគណៈរដ្ឋមន្ត្រី</div> -->
        <div
          class="h-3 text-left text-xxxs font-moul text-yellow-400 leading-4"
        >
          ទីស្ដីការគណៈរដ្ឋមន្ត្រី
        </div>
        <div
          class="h-5 text-left text-xxs font-moul -mt-1 pt-1 text-white leading-5"
        >
    ...
    <div
      class="ocm-bottom-menu w-full flex flex-row-reverse items-center mr-12"
    >
      <!-- <div 
...
export default {
  ...
  setup() {
    ...
    const systemName = computed(() => {
      return store.state.system.name != ""
        ? store.state.system.name
        : "ប្រព័ន្ធបណ្ដុំឯកសារ";
    });
    const username = computed(() => {
      let user = getUser();
      return user !== null && user.people != null
