                ...
                <table class="w-full border-collapse min-w-[980px]">
                    ...
                    <tbody>
                        <tr v-for="(row, idx) in rows" :key="row._key" :class="rowEdited(row) ? 'bg-[#FEE2E2]' : 'bg-white'">
                            ...
                            <!-- File upload cell (clone of childrenInformation style) -->
                            <td class="p-2 border-b border-[#E5E7EB]">
                                <div class="certificate-upload">
                                    <label :for="`medal-file-${row._key}`" class="certificate-upload-trigger">ឯកសារ</label>
                                    <input
                                        :id="`medal-file-${row._key}`"
                                        type="file"
                                        class="hidden"
                                        accept=".pdf,image/*"
                            ...
                            <!-- Actions: PDF preview + delete (clone from childrenInformation) -->
...
export default {
    ...
    setup(props, { emit }) {
        ...
        const togglePdfModal = (row) => {
            // Behave like childrenInformation: set record when provided, just toggle when called from onClose
        ...
        const downloadDocument = async (row) => {
            try {
                ...
                if (!fileUrl) {
                    throw new Error('Medal file URL is missing')
                }

                triggerBrowserDownload(
                    fileUrl,
                    res?.data?.filename || row.fileName || `medal-${row.id}.pdf`
                )
            } catch (error) {
        ...
        const persistChanges = async () => {
            ...
            for (const row of rows.value) {
                ...
                if (row.file && row.id) {
                    ...
                    await store.dispatch('officermedalhistory/upload', formData)
                }
            }
