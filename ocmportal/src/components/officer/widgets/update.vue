<template>
  ...
  <div class="vcb-pop-create font-ktr">
    ...
    >
      <n-card
        class="w-10/12 sm:w-3/4 md:w-8/12 lg:w-3/5 xl:w-7/12"
      ...
      >
        ...
        <div class="crud-create-form w-full border-t">
          ...
          <n-tabs type="segment" animated>
            ...
            <!-- NEW: តួនាទី tab (position) -->
            <n-tab-pane name="position-detail" tab="តួនាទី">
              <div class="mx-auto p-4 flex-wrap">
                <div class="crud-form-panel w-full flex flex-wrap">
                  <n-form
                    v-if="record != null"
                    ...
                    ref="formRef"
                  >
                    <n-form-item label="តួនាទី" class="w-full mb-4 p-1">
                      <n-select
                    ...
                    <n-form-item
                      label="តួនាទី​ ព័ត៌មានក្នុងអង្គភាព​​ (PDF)"
                      class="w-full p-1"
                    >
                      <n-upload
                        :action="uploadAction"
                        ...
                        :multiple="false"
                        show-file-list
                        @change="onPositionUploadChange"
                      >
                        <n-button type="primary"
                          >ជ្រើសឯកសារ PDF និងផ្ទុកឡើង</n-button
                        >
                      </n-upload>
                      <div v-if="record.position_document" class="mt-2">
                        <n-space>
                          <a
                            :href="record.position_document"
                            target="_blank"
                            download
                          >
                          ...
                          <a :href="record.position_document" target="_blank">
                            <n-button type="default" size="small"
                              >ពិនិត្យឯកសារ</n-button
                            >
                          </a>
                        </n-space>
                      </div>
            ...
            <!-- NEW: ក្របខណ្ឌ tab -->
            <n-tab-pane name="krobkhan-detail" tab="ក្របខណ្ឌ">
              <div class="mx-auto p-4 flex-wrap">
                <div class="crud-form-panel w-full flex flex-wrap">
                  <n-form
                    v-if="record != null"
                  ...
                  >
                    ...
                    <n-form-item label="ឯកសារ (PDF)" class="w-full p-1">
                      <n-upload
                        :action="uploadAction"
                        :headers="uploadHeaders"
                        name="file"
                        ...
                        :multiple="false"
                        show-file-list
                        @change="onKrobkhanUploadChange"
                      >
                        <n-button type="primary"
                          >ជ្រើសឯកសារ PDF និងផ្ទុកឡើង</n-button
                        >
                      </n-upload>
                      <div v-if="record.krobkhan_document" class="mt-2">
                        <n-space>
                          <a
                            :href="record.krobkhan_document"
                            target="_blank"
                            download
                          >
                            <n-button type="info" size="small"
                          ...
                          <a :href="record.krobkhan_document" target="_blank">
                            <n-button type="default" size="small"
                              >ពិនិត្យឯកសារ</n-button
                            >
                          </a>
              ...
              <div class="mx-auto p-4 flex-wrap">
                <div class="crud-form-panel w-full flex flex-wrap">
                  ...
                  >
                    ...
                    <div
                      v-if="record.code != undefined && record.code.length > 0"
                      class="w-full mb-4"
                    >
                    ...
                    <!-- <n-form-item label="អង្គភាព" path="organization" class="w-full mb-4" >
                       ...
                       -->
                    <!-- moved 'តួនាទី' to the dedicated តួនាទី tab -->
  ...
  setup(props) {
        ...
        .dispatch(
          props.model.name + "/update",
          {
            ...
            unofficial_position_id:
              parseInt(selectedUnofficialPosition.value) > 0
            ...
            official_date:
              official_date.value != null
                ? dateFormat(new Date(official_date.value), "yyyy-mm-dd")
                : dateFormat(new Date(), "yyyy-mm-dd"),
            unofficial_date:
              unofficial_date.value != null
                ? dateFormat(new Date(unofficial_date.value), "yyyy-mm-dd")
                : dateFormat(new Date(), "yyyy-mm-dd"),
            ank:
            ...
            people: {
              ...
              dob:
                dob.value != null
                  ? dateFormat(new Date(dob.value), "yyyy-mm-dd")
                  : dateFormat(new Date(), "yyyy-mm-dd"),
              ...
              father_dob:
                fatherDob.value != null
                  ? dateFormat(new Date(fatherDob.value), "yyyy-mm-dd")
                  : dateFormat(new Date(), "yyyy-mm-dd"),
              ...
              mother_dob:
                motherDob.value != null
                  ? dateFormat(new Date(motherDob.value), "yyyy-mm-dd")
                  : dateFormat(new Date(), "yyyy-mm-dd"),
    ...
    function getPositionStructures() {
      store
        ...
        .then((res) => {
          if (res.data.ok) {
            organizationStructurePositions.value = res.data.records
              .filter((p) => {
              ...
              .map((p) => {
                return {
                  label:
                    p.organizationStructure.organization.name +
                    " - " +
                    p.position.name,
                  value: p.id,
                };
    ...
    // upload state & handler for ក្របខណ្ឌ document
    const uploadAction = ref(
      import.meta.env.VITE_API_SERVER + "/api/admin/officers/upload/pdf",
    );
    const uploadHeaders = computed(() => ({
      Authorization: store.getters["auth/getAuthorization"] || "",
    }));
    const krobkhanFile = ref(null);
    function onKrobkhanUploadChange(fileInfo) {
      try {
        if (
          fileInfo &&
          fileInfo.file &&
          fileInfo.file.response &&
          fileInfo.file.response.url
        ) {
          props.record.krobkhan_document = fileInfo.file.response.url;
        }
        krobkhanFile.value = fileInfo.file;
      } catch (e) {
        console.log("krobkhan upload change", e);
      }
    }
    // upload state & handler for តួនាទី (position) document
    const positionFile = ref(null);
    function onPositionUploadChange(fileInfo) {
      try {
        if (
          fileInfo &&
          fileInfo.file &&
          ...
          props.record.position_document = fileInfo.file.response.url;
        }
        positionFile.value = fileInfo.file;
      } catch (e) {
        console.log("position upload change", e);
      }
    }
    ...
    function initial() {
      ...
      selectedPosition.value =
        props.record.position != null && props.record.position != undefined
          ...
          : null;
      selectedUnofficialPosition.value =
        props.record.current_job != null &&
        props.record.current_job != undefined
          ? props.record.current_job.unofficial_position_id
          : null;
    ...
    return {
      ...
      uploadHeaders,
      krobkhanFile,
      onKrobkhanUploadChange,
      positionFile,
      onPositionUploadChange,
    };
  },
};
</script>
