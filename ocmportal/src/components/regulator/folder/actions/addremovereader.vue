<template>
  ...
  <div class="vcb-pop-create font-ktr">
    <n-modal
      ...
      :on-before-leave="tableClearBeforeLeave"
      transform-origin="center"
      :on-after-enter="initial"
    >
      <n-card
        class="w-4/5 font-pvh text-xl relative"
        :title="'ប្រតិបត្តិការផ្សេងទៀតរបស់ ' + model.title"
        :bordered="false"
        size="small"
      >
          ...
          <div class="flex-grow action-buttons flex-row-reverse flex">
            <div class="w-2/5 relative">
              <input
                ...
                class="bg-gray-100 px-2 h-9 my-1 w-full rounded border border-gray-200 focus:border-blue-600 hover:border-blue-600"
                placeholder="ស្វែងរក"
              />
              <svg
                class="absolute right-1 top-2 text-gray-400 hover:text-blue-700 cursor-pointer"
        ...
        <div
          v-if="!table.loading"
          class="vcb-table-panel flex flex-wrap justify-center p-4"
        >
          <div
            v-for="(user, index) in table.records.matched"
            :key="index"
            class="p-4 hover:shadow-md duration-300 cursor-pointer m-4 relative w-60 text-xs flex flex-wrap border border-gray-200"
            @click="
              user.regulators == undefined
                ? addReader(user)
                : user.regulators.find((r) => r.id == record.id)
                  ? removeReader(user)
                  : addReader(user)
            "
          >
            ...
            <div
              v-if="user.regulators != undefined"
        ...
        <div class="vcb-table-pagination">
          <div
            @click="loadmore()"
            class="border rounded py-2 px-8 m-4 w-60 text-center mx-auto hover:border-blue-500 focus:border-blue-500 duration-300 cursor-pointer bg-gray-100"
          >
            បង្ហាញបន្ថែម
          </div>
        </div>
        ...
        <div
          v-if="table.loading"
          class="table-loading absolute left-0 top-0 right-0 bottom-0 bg-white bg-opacity-75"
        >
...
export default {
  ...
  setup(props) {
      ...
      store
        ...
        .then((res) => {
          if (table.loadmore) {
            ...
            table.records.matched = table.records.matched.concat(
              res.data.records,
            );
          } else {
    ...
    function addReader(user) {
      store
        .dispatch("regulator/addReader", {
          user_id: user.id,
          document_id: props.record.id,
        })
        .then((res) => {
    ...
    function removeReader(user) {
      store
        .dispatch("regulator/removeReader", {
          user_id: user.id,
          document_id: props.record.id,
        })
        .then((res) => {
