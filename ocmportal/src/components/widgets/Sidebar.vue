  ...
  <div class="app_snav">
    <div class="app_card_w">
      ...
      <div
        ...
        @click="toggleSnav"
        :class="{ 'is-active': isSnavOpen, 'is-busy': isSnavAnimating }"
      >
        <span class="apico">
          <svg
...
export default {  
  ...
  setup(props){
    ...
    const isSnavAnimating = ref(false)
    const snavAnimationMs = 120

    function applySnavState(open) {
      isSnavOpen.value = open
      window.requestAnimationFrame(() => {
        document.body.classList.toggle('app_snav_open', open)
      })
    }

    function toggleSnav() {
      if (isSnavAnimating.value) return
      isSnavAnimating.value = true
      applySnavState(!isSnavOpen.value)
      window.setTimeout(() => {
        isSnavAnimating.value = false
      }, snavAnimationMs)
      window.setTimeout(() => {
        localStorage.setItem('app_snav_open', String(isSnavOpen.value))
      }, 0)
    }
...
body:not(.app_snav_open) .app_snav .app_card .apico {
...
.app_snav .app_card:hover .apico,
.app_snav .app_card.app_active .apico {
  background: #ffcd00;
  color: #000;
}

.app_snav .app_card:hover .app_txt,
.app_snav .app_card.app_active .app_txt {
  color: #000;
}

.app_snav .app_card.app_active .apico svg {
  color: #000;
}

.app_card.app_toggle .apico {
