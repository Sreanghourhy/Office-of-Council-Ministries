...
/**
 * Get Khmer lunar date parts for a given date (default: today).
 * Use for header line and signature auto-fill (ថ្ងៃ ទី ខែ ឆ្នាំ = weekday, day+phase, lunar month, animal+Sak).
 * @param {Date} [date=new Date()]
 * @returns {{ headerText: string, weekday: string, day: string, month: string, year: string }}
 */
function getKhmerDateParts(date = new Date()) {
    try {
        const result = fromDate(date);
        const fmt = (token) => format(result, token);
        return {
            headerText: format(result, 'ថ្ងៃ: W dN ខែ: m ឆ្នាំ: a e ព.ស. b'),
            weekday: fmt('W'),
            day: fmt('dN'),
            month: fmt('m'),
            year: fmt('a e'),
        ...
        return { headerText: '', weekday: '', day: '', month: '', year: '' };
    }
}
